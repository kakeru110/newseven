...
通算: 売上 3,901,313円 / 営業損益 372,203円 / 限界利益率 43.3% / 平均稼働率 74.5% — **全項目一致**。

2026年3月は 2026-08-29 に収支表PDFを入手し、逆算値・推定値から実額へ差し替えました
（明細25件の合計がPDFの合計欄と1円まで一致することを確認済み）。

2026年8月は 2026-09-01 付の清掃請求書で清掃・ごみを実額に差し替えました（清掃20回→22回、ごみ44袋→42袋）。
清掃単価の実績平均も 8,540円→8,624円（96滞在）に更新したため、損益分岐価格 P(N) が全体に上がっています。
2026-09-09 に8月の収支表PDFも入手し、運営サポート料を実額（91,764円）に差し替え、チャネル別内訳も確定しました。
残るは光熱費・日用品で、単価マスタからの推計値のままです。

2026-09-09 に `nightsActual` を Beds24 のカレンダー基準へ直しました。収支表の「宿泊日数」は
到着月ベース（チェックイン月に滞在まるごとを計上）で、それを実泊数として取り込んでいたためです
（5月23→21、6月23→20、7月24→23）。**平均稼働率 77.4% → 74.5%、ALOS 1.36 → 1.31泊。**
売上・限界利益率・営業損益は変わりません。

**収支表PDFの合計欄は明細と1円ずれることがあります。** 8月分は OTA手数料が合計欄 122,814円に対し
明細20件の合計 122,815円。各行は `commission = 売上 × 料率` で再現できるため明細側を採りました。
`npm run verify` が明細合計との一致を検算します。
...
| § | セクション | 内容 |
|---|---|---|
| 12-1 | サマリーカード | 確定分（収支表PDF由来）の通算＋予約済み（Beds24由来）の売上。8月が仮値である旨のバッジ |
...
- 値下げを考える前に「販売可能」列を見てください。住宅宿泊事業法の180日枠が
  ...
  2026-09-07 時点では確定済みの予約だけで **4泊超過** しており、
  営業日数カウンターに超過している泊と許可の期限を出しています。
...
別途取得し、売上には含めずに件数だけを数えます（2026-09-07 時点で37件・19.4%）。
Booking.com エクストラネットの表示（20.0〜21.77%）と比べるときは分母も Booking.com だけにしてください
（27/129件＝20.9%で一致します）。直販フローの動作確認で入れた自社のテスト予約は
`seed.testBookings.ids` に列挙して集計から除いています。

## 値上げシミュレーター

「単価を上げて稼働が落ちても利益は増えるのか」を判断するための計算です（`src/lib/pricing.js`）。

実績を基準点とし、そこからの**差分だけ**を単価マスタで計算します。モデルで金額を一から
組み立てると、清掃の立替控除や光熱費の実額との差が基準点でズレるためです。
基準点（変化なし）では実績と完全に一致します。
...
清掃費が1滞在あたり固定なので、泊数が減るとコストも減ります。これが許容幅を広げています。

**下限（ミニマム価格）はすでに守れています。** 2026年2〜8月の122予約のうち損益分岐価格を
割ったのは2件だけ（2月・不足4,553円）で、3月以降はゼロ。伸びしろは上振れ側にあります。

### 月別の値上げ推奨

稼働率から機械的に判定します（高稼働88%以上 → +15% ／ 中稼働70%以上 → +10% ／ それ未満は据え置き）。

**2026年2月・3月は立ち上げ期として判断から除外します。** 営業開始直後でレビューが少なく、
レビュー獲得のために意図的に単価を下げていた期間なので、低単価を「安すぎる」と評価してはいけません。
該当月は `seed.pricingContext.launchMonths` に持たせています。
...
住宅宿泊事業法の年間営業日数を Beds24 の予約データから日単位で数え、上限に達する日を予測します。

| | |
|---|---|
| カウント期間 | 毎年4月1日正午 〜 翌年4月1日正午 |
| 上限 | 180日（1泊＝1日） |
| 2026-08-29 時点の実績 | 115日 |
| 既存予約を含む確定分 | 178日 |
| **180泊目を迎える日**（24泊/月のペース） | **2026-11-21** |

上限に達するとその年度は営業できません。**旅館業の許可取得が間に合うかを常時監視する**のがこの機能の目的です。
予約が入るたびに到達予測日が更新され、残り30日を切ると警告色になります。

```bash
npm run verify:regulation
```

境界日の解釈で±1日ずれうるので、自治体への定期報告の値と突き合わせて確認してください。
